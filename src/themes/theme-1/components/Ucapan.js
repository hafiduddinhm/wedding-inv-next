...
const Ucapan = forwardRef((props, sectionRef) => {
  ...
  const fetchData = async () => {
    try {
      const response = await axios.get(data.rsvpEndpoint);
  ...
  const handleChange = (event) => {
    ...
    if (name === 'kehadiran') {
      ...
      if (value.length <= MAX_LENGTH) setNama(value);
    } else if (name === 'ucapan') {
      setUcapan(value);
    }
  };
  ...
  const handleSubmit = (event) => {
    ...
    axios
      .post(data.rsvpEndpoint, formdata, {
  ...
  const styles = {
    ...
    btn: {
      submit: {
        ...
        '&:hover': {backgroundColor: '#875B57', boxShadow: 'none'},
      },
      cancel: {
        borderRadius: 30,
        ...
        >
          <TextField
            ...
            InputProps={{
              ...
              style: {borderRadius: 20, backgroundColor: '#F3D7D7'},
            }}
